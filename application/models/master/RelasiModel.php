...
class RelasiModel extends CI_Model
{
    ...
    {
        $hasil = $this->db->query('SELECT * FROM MASTER_RELASI WHERE RECORD_STATUS="AKTIF" AND PERUSAHAAN_KODE="' . $this->session->userdata('PERUSAHAAN_KODE') . '" ORDER BY MASTER_RELASI_QR_ID DESC ')->result();
        return $hasil;
    }
