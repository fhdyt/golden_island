...
class JaminanModel extends CI_Model
{
    ...
    public function list()
    {
        ...
        $hasil = $this->db->query('SELECT * FROM FAKTUR_JAMINAN WHERE ' . $filter . ' RECORD_STATUS="AKTIF" AND PERUSAHAAN_KODE="' . $this->session->userdata('PERUSAHAAN_KODE') . '" ORDER BY FAKTUR_JAMINAN_TANGGAL DESC ')->result();
    ...
    public function surat_jalan_list()
    {
        ...
        SURAT_JALAN
        WHERE SURAT_JALAN_JENIS="penjualan" AND SURAT_JALAN_REALISASI_STATUS="selesai" AND RECORD_STATUS="AKTIF" AND PERUSAHAAN_KODE="' . $this->session->userdata('PERUSAHAAN_KODE') . '"')->result();
        foreach ($hasil as $row) {
