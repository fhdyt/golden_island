...
<script>
    ...
    function relasi_list() {
        $.ajax({
            type: 'ajax',
            url: "<?php echo base_url() ?>index.php/akuntansi/hutang/detail_list_hutang?id=<?= $this->uri->segment('4') ?>",
            async: false,
            dataType: 'json',
            success: function(data) {
                ...
                if (data.length === 0) {
                    ...
                    for (i = 0; i < data.length; i++) {
                        total_hutang += parseInt(data[i].HUTANG_DEBET) - parseInt(data[i].HUTANG_KREDIT)
