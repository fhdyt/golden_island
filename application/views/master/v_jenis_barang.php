    ...
    <div class="content">
        <div class="container-fluid">
            <div class="card card-default color-palette-box">
                <div class="card-body">
                    <button type="button" class="btn btn-secondary btn_jenis_barang mb-2">Tambah Jenis Barang</button>
...
<script>
    ...
    function jenis_barang_list() {
        $.ajax({
            type: 'ajax',
            url: "<?php echo base_url() ?>index.php/master/jenis_barang/list",
            async: false,
            dataType: 'json',
                ...
                if (data.length === 0) {
                    ...
                    for (i = 0; i < data.length; i++) {
                        $("tbody#zone_data").append("<tr class=''>" +
                            "<td>" + no++ + ".</td>" +
                            "<td>" + data[i].MASTER_JENIS_BARANG_NAMA + "</td>" +
                            "<td><a class='btn btn-danger btn-sm' onclick='hapus(\"" + data[i].MASTER_JENIS_BARANG_ID + "\")'><i class='fas fa-trash'></i></a> " +
                            "<a class='btn btn-warning btn-sm' onclick='detail(\"" + data[i].MASTER_JENIS_BARANG_ID + "\")'><i class='fas fa-edit'></i></a> " +
                            "<a class='btn btn-primary btn-sm'  href='<?php echo base_url(); ?>master/jenis_barang/detail_barang/" + data[i].MASTER_JENIS_BARANG_ID + "' ><i class='fas fa-file-alt'></i> Detail</a></td>" +
                            "</tr>");
                    }
                }
            },
    ...
    function hapus(id) {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    type: 'ajax',
                    url: '<?php echo base_url() ?>index.php/master/jenis_barang/hapus/' + id,
                    async: false,
                    dataType: 'json',
                    success: function(data) {
